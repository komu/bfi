...
(library (bf registers)
  (export register-maker make-register register? register-offset define-scratch-register)
  (import (rnrs base)
...
(define (register-maker)
  (let ([counter (make-counter *defined-scratch-registers*)])
    (lambda () (make-register (counter)))))

(define (make-scratch-register)
...
(define-syntax define-scratch-register
  (syntax-rules()
    [(_ name)
     (define name (make-scratch-register))]))
